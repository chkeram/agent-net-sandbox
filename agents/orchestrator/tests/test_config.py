...
class TestSettings:
    ...
    def test_default_settings(self):
        ...
        assert settings.llm_provider == "both"
        assert settings.default_model_temperature == 0.7
        assert settings.discovery_interval_seconds == 60  # From test fixture override
    ...
    def test_both_providers_configuration(self):
    ...
    def test_production_validation(self):
        ...
        with pytest.raises(ValueError, match="Anthropic API key is required in production"):
            get_settings_for_testing(
                environment="production",
                llm_provider="anthropic",
    ...
    def test_docker_availability_check(self):
        ...
        settings = get_settings_for_testing(docker_socket_path="/tmp/test_socket")
        ...
        # Non-existent socket
        settings = get_settings_for_testing(docker_socket_path="/nonexistent/socket")
    ...
    def test_settings_caching(self):
        ...
        test_settings1 = get_settings_for_testing(debug=True)
        test_settings2 = get_settings_for_testing(debug=True)
    ...
    def test_enum_validation(self):
        ...
        with pytest.raises(ValidationError):
            get_settings_for_testing(llm_provider="invalid_provider")
        
        with pytest.raises(ValidationError):
            get_settings_for_testing(environment="invalid_env")
        
        with pytest.raises(ValidationError):
            get_settings_for_testing(log_level="INVALID_LEVEL")
